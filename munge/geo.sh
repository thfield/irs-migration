...
if [ ! -f raw/national_county.txt ]; then
  curl -o raw/national_county.txt 'https://www2.census.gov/geo/docs/reference/codes/files/national_county.txt'
fi
echo 'state,statefp,countyfp,name,type' > fipscodes.csv
cat raw/national_county.txt | tr -d '\r' >> fipscodes.csv # get rid of CRLF lineendings
cat ../munge/extrafips.csv >> fipscodes.csv
